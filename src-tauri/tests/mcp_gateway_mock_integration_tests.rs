...
/// Initialize a gateway session for the given client_id.
///
/// The MCP gateway rejects non-initialize requests with
/// `"Session not initialized"` until the client has sent an `initialize`
/// call. Tests that only exercise the post-init paths (tools/list,
...
async fn init_session(gateway: &McpGateway, client_id: &str, allowed_servers: Vec<String>) {
    ...
    let _ = gateway
        .handle_request(client_id, allowed_servers, vec![], init)
        .await;
}
...
async fn test_gateway_initialize_merges_capabilities() {
    ...
    let response = gateway
        .handle_request("test-client", allowed_servers, vec![], request)
        .await
        .unwrap();
...
async fn test_gateway_tools_list_merges_and_namespaces() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    init_session(&gateway, "test-client", allowed_servers.clone()).await;
    let response = gateway
        .handle_request("test-client", allowed_servers, vec![], request)
        .await
        .unwrap();
...
async fn test_gateway_handles_all_servers_failing() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    let result = gateway
        .handle_request("test-client-fail", allowed_servers, vec![], request)
        .await;
...
async fn test_gateway_handles_json_rpc_error() {
    ...
    let allowed_servers = vec!["server1".to_string()];
    let result = gateway
        .handle_request("test-client-error", allowed_servers, vec![], request)
        .await;
...
async fn test_resources_read_routes_by_uri() {
    ...
    init_session(&gateway, "test-client-res", allowed_servers.clone().clone()).await;
    let _ = gateway
        .handle_request(
            "test-client-res",
            allowed_servers.clone(),
...
async fn test_resources_list_with_templates() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    init_session(&gateway, "test-client-templates", allowed_servers.clone()).await;
    let response = gateway
...
async fn test_prompts_list_with_arguments() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    init_session(&gateway, "test-client-args", allowed_servers.clone()).await;
    let response = gateway
        .handle_request("test-client-args", allowed_servers, vec![], request)
        .await
        .unwrap();
...
async fn test_tools_list_handles_duplicates() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    init_session(&gateway, "test-client-dup", allowed_servers.clone()).await;
    let response = gateway
        .handle_request("test-client-dup", allowed_servers, vec![], request)
        .await
        .unwrap();
...
async fn test_tools_call_handles_error_response() {
    ...
    init_session(&gateway, "test-client-err", allowed_servers.clone().clone()).await;
    let _ = gateway
        .handle_request(
            "test-client-err",
            allowed_servers.clone(),
...
async fn test_session_reuse() {
    ...
    init_session(&gateway, client_id, allowed_servers.clone().clone()).await;
    let _ = gateway
        .handle_request(client_id, allowed_servers.clone(), vec![], request1)
        .await
        .unwrap();
    ...
    let response2 = gateway
        .handle_request(client_id, allowed_servers, vec![], request2)
        .await
        .unwrap();
...
async fn test_concurrent_clients() {
    ...
    // Client 1 request
    init_session(&gateway, "client1", vec!["server1".to_string()].clone()).await;
    ...
    // Client 2 request
    init_session(&gateway, "client2", vec!["server2".to_string()]).await;
    let response2 = gateway
...
async fn test_notification_invalidates_tools_cache() {
    ...
    let response2 = gateway
        .handle_request("test-client-notif", allowed_servers, vec![], request)
        .await
        .unwrap();
...
async fn test_notification_forwarded_to_client() {
    ...
    init_session(&gateway, "test-client-notif4", vec!["server1".to_string()]).await;
...
async fn test_initialize_latency() {
    ...
    let _ = gateway
        .handle_request("test-client-perf", allowed_servers, vec![], request)
        .await
        .unwrap();
...
async fn test_tools_list_cached_latency() {
    ...
    let start = Instant::now();
    let _ = gateway
        .handle_request("test-client-perf2", allowed_servers, vec![], request)
        .await
        .unwrap();
    let elapsed = start.elapsed();
...
async fn test_tools_list_uncached_latency() {
    ...
    let start = Instant::now();
    init_session(&gateway, "test-client-perf3", allowed_servers.clone()).await;
    let _ = gateway
        .handle_request("test-client-perf3", allowed_servers, vec![], request)
        .await
        .unwrap();
    let elapsed = start.elapsed();
...
async fn test_concurrent_sessions_memory() {
    ...
    for i in 0..50 {
        ...
        let handle = tokio::spawn(async move {
            gateway_clone
                .handle_request(&client_id, allowed_clone, vec![], request_clone)
                .await
        });
...
async fn test_http_500_error() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    let response = gateway
        .handle_request("test-client-500", allowed_servers, vec![], request)
        .await;
...
async fn test_tools_list_partial_failure() {
    ...
    let allowed_servers = vec!["server1".to_string(), "server2".to_string()];
    init_session(&gateway, "test-client-partial", allowed_servers.clone()).await;
    let response = gateway
