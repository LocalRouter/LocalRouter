...
async fn logging_middleware(
    ...
    use self::middleware::client_auth::LoggedClientId;
    ...
    if status.is_success() {
        info!("{} {} - {} ({:?}) [{}] client={}", method, uri, status, elapsed, peer_str, client);
    } else {
        error!("{} {} - {} ({:?}) [{}] client={}", method, uri, status, elapsed, peer_str, client);
    }
