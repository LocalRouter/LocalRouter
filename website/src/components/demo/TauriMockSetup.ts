...
const mockHandlers: Record<string, (args?: any) => unknown> = {
  ...
  'preview_catalog_compression': (args) => {
    ...
    return {
      welcome_message: `Unified MCP Gateway.\n\n<context-management>\n- \`ctx_search\` (tool)\n\nUse ctx_search to discover MCP capabilities and retrieve compressed content.\n</context-management>\n\n<github>\n- 23 tools${isMidThreshold ? ' — ctx_search(source="catalog:github") to view full details' : ''}\n</github>\n\n<filesystem>\n- filesystem__read_file\n- filesystem__write_file\n- filesystem__edit_file\n${isLowThreshold ? '- 7 more tools — ctx_search(source="catalog:filesystem") to explore\n' : '- filesystem__create_directory\n- filesystem__list_directory\n- filesystem__directory_tree\n- filesystem__move_file\n- filesystem__search_files\n- filesystem__get_file_info\n- filesystem__read_multiple_files\n'}</filesystem>\n\n<postgresql>\n- postgres__query\n- postgres__execute\n- postgres__list_tables\n- postgres__describe_table\n- postgres__list_schemas\n- postgres__explain_query\n</postgresql>\n\n<slack>\n- 8 tools${isMidThreshold ? ' — ctx_search(source="catalog:slack") to view full details' : ''}\n</slack>\n`,
      welcome_message_uncompressed: `Unified MCP Gateway.\n\n<context-management>\n- \`ctx_search\` (tool)\n\nUse ctx_search to discover MCP capabilities and retrieve compressed content.\n</context-management>\n\n<github>\n**GitHub** — GitHub's official MCP server for repository management, issues, pull requests, code search, actions workflows, and code security scanning.\n\n- github__issue_read — Read issue details, comments, sub-issues, or labels\n- github__issue_write — Create, update, close, or reopen issues\n- github__search_issues — Search issues across repositories using GitHub search syntax\n- github__list_issues — List issues in a repository with filtering by state, labels, assignee\n- github__add_issue_comment — Add a comment to an issue or pull request\n- github__sub_issue_write — Create or manage sub-issues for tracking work breakdown\n- github__pull_request_read — Get PR details, diff, status, files, reviews, or review comments\n- github__create_pull_request — Create a new pull request with title, head branch, and base branch\n- github__update_pull_request — Update PR title, body, state, base branch, or maintainer access\n- github__merge_pull_request — Merge a PR using merge, squash, or rebase strategy\n- github__add_pull_request_review_comment — Add an inline review comment on a PR diff\n- github__get_file_contents — Retrieve file or directory contents from a repository\n- github__create_or_update_file — Create or update a single file in a repository\n- github__push_files — Commit and push multiple files in a single commit\n- github__search_code — Search code across repositories using GitHub code search syntax\n- github__search_repositories — Search repositories by name, description, language, or topic\n- github__list_commits — List commits on a branch with optional path and author filtering\n- github__list_branches — List branches in a repository with optional search filtering\n- github__create_branch — Create a new branch from an existing ref\n- github__list_workflow_runs — List GitHub Actions workflow runs with status and branch filtering\n- github__get_workflow_run_logs — Retrieve logs for a specific workflow run\n- github__rerun_workflow — Re-run a failed or completed workflow\n- github__get_code_scanning_alerts — Get code scanning alerts for a repository\n</github>\n\n<filesystem>\n**Filesystem** — Secure filesystem operations with configurable access controls.\n\n- filesystem__read_file — Read the complete contents of a file as UTF-8 text or base64\n- filesystem__write_file — Create or overwrite a file, creating parent directories if needed\n- filesystem__edit_file — Apply targeted edits using diff-like oldText/newText format\n- filesystem__create_directory — Create a directory and parent directories recursively\n- filesystem__list_directory — List entries in a directory with [FILE] or [DIR] prefixes\n- filesystem__directory_tree — Return a recursive tree structure up to configurable depth\n- filesystem__move_file — Move or rename a file or directory\n- filesystem__search_files — Search for files matching a glob pattern recursively\n- filesystem__get_file_info — Get file metadata: size, timestamps, permissions, type\n- filesystem__read_multiple_files — Read several files at once, more efficient than individual calls\n</filesystem>\n\n<postgresql>\n**PostgreSQL** — Database integration for executing queries, managing schemas, and analyzing performance.\n\n- postgres__query — Execute a read-only SQL query (SELECT, EXPLAIN, SHOW) with parameterized values\n- postgres__execute — Run write SQL (INSERT, UPDATE, DELETE, CREATE, ALTER, DROP)\n- postgres__list_schemas — List all schemas in the database with descriptions\n- postgres__list_tables — List tables in a schema with row counts and descriptions\n- postgres__describe_table — Get column definitions, indexes, foreign keys, and constraints\n- postgres__explain_query — Run EXPLAIN ANALYZE on a query within a rolled-back transaction\n</postgresql>\n\n<slack>\n**Slack** — Workspace integration for messaging, channel management, and conversation search.\n\n- slack__send_message — Post a message to a channel or DM with Slack mrkdwn formatting\n- slack__list_channels — List workspace channels with IDs, names, topics, and member counts\n- slack__search_messages — Full-text search across messages with search modifiers\n- slack__get_thread — Retrieve all replies in a thread by channel ID and timestamp\n- slack__get_channel_history — Fetch recent messages with optional time range filtering\n- slack__get_users — List workspace members with display names, emails, and status\n- slack__add_reaction — Add an emoji reaction to a message\n- slack__upload_file — Upload a file to a channel with optional initial comment\n</slack>\n`,
      uncompressed_size: uncompressedSize,
      compressed_size: compressedSize,
      compressed_descriptions_count: isMidThreshold ? 3 : isLowThreshold ? 5 : 0,
      deferred_items_count: isLowThreshold ? 1 : 0,
      truncated_servers_count: 0,
      servers: [
        {
          name: 'Context Management', is_virtual: true,
          tool_names: ['ctx_search'], resource_names: [], prompt_names: [],
          description: 'Use ctx_search to discover MCP capabilities and retrieve compressed content.',
          instructions: null, compression_state: 'visible',
          tools: [{
            name: 'ctx_search', description: 'Search the indexed MCP catalog and tool execution results. Returns matching content from compressed descriptions, deferred tools, and previously executed tool outputs.',
        ...
        },
        {
          ...
          tools: [
            { name: 'github__issue_read', description: 'Read issue details, comments, sub-issues, or labels from a GitHub repository. Supports multiple methods: get (issue details), get_comments (comments list), get_sub_issues (sub-issue tree), get_labels (label list).', input_schema: { type: 'object', properties: { owner: { type: 'string', description: 'Repository owner (user or organization)' }, repo: { type: 'string', description: 'Repository name' }, issue_number: { type: 'number', description: 'Issue number' }, method: { type: 'string', enum: ['get', 'get_comments', 'get_sub_issues', 'get_labels'], description: 'The operation to perform on the issue' } }, required: ['owner', 'repo', 'issue_number'] } },
            { name: 'github__issue_write', description: 'Create, update, close, or reopen issues in a GitHub repository. For creating: requires owner, repo, title. For updating: requires owner, repo, issue_number.', input_schema: { type: 'object', properties: { owner: { type: 'string', description: 'Repository owner' }, repo: { type: 'string', description: 'Repository name' }, method: { type: 'string', enum: ['create', 'update', 'close', 'reopen'], description: 'Operation to perform' }, title: { type: 'string', description: 'Issue title (required for create)' }, body: { type: 'string', description: 'Issue body in Markdown' }, issue_number: { type: 'number', description: 'Issue number (required for update/close/reopen)' }, labels: { type: 'array', items: { type: 'string' }, description: 'Labels to add' }, assignees: { type: 'array', items: { type: 'string' }, description: 'GitHub usernames to assign' } }, required: ['owner', 'repo', 'method'] } },
            { name: 'github__search_issues', description: 'Search issues and pull requests across GitHub repositories using the GitHub search syntax. Supports qualifiers like repo:, is:, label:, author:, state:, and more.', input_schema: { type: 'object', properties: { query: { type: 'string', description: 'Search query using GitHub search syntax (e.g., "repo:owner/name is:open label:bug")' }, per_page: { type: 'number', description: 'Results per page (max 100, default 30)' }, page: { type: 'number', description: 'Page number for pagination' } }, required: ['query'] } },
            { name: 'github__list_issues', description: 'List issues in a repository with optional filtering by state, labels, assignee, milestone, and sort order.', input_schema: { type: 'object', properties: { owner: { type: 'string', description: 'Repository owner' }, repo: { type: 'string', description: 'Repository name' }, state: { type: 'string', enum: ['open', 'closed', 'all'], description: 'Filter by state (default: open)' }, labels: { type: 'string', description: 'Comma-separated list of label names' }, assignee: { type: 'string', description: 'GitHub username or "none" for unassigned' }, sort: { type: 'string', enum: ['created', 'updated', 'comments'], description: 'Sort field' }, direction: { type: 'string', enum: ['asc', 'desc'], description: 'Sort direction' }, per_page: { type: 'number', description: 'Results per page (max 100)' } }, required: ['owner', 'repo'] } },
        ...
        },
        {
          ...
          tools: [
            { name: 'filesystem__read_file', description: 'Read the complete contents of a file. Returns text content with UTF-8 encoding. For binary files, returns base64-encoded content.', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Absolute path to the file to read' } }, required: ['path'] } },
            { name: 'filesystem__write_file', description: 'Create or overwrite a file with the given content. Creates parent directories if they don\'t exist. Content must be a string (use base64 for binary data).', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Absolute path to the file to write' }, content: { type: 'string', description: 'Content to write to the file' } }, required: ['path', 'content'] } },
            { name: 'filesystem__edit_file', description: 'Apply targeted edits to a file using a diff-like format. Supports multiple edits in a single call. Each edit specifies oldText (must match exactly) and newText. More reliable than write_file for partial modifications.', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Absolute path to the file to edit' }, edits: { type: 'array', items: { type: 'object', properties: { oldText: { type: 'string', description: 'Text to find (must match exactly)' }, newText: { type: 'string', description: 'Replacement text' } }, required: ['oldText', 'newText'] }, description: 'Array of edit operations to apply' }, dryRun: { type: 'boolean', description: 'Preview changes without applying them' } }, required: ['path', 'edits'] } },
            { name: 'filesystem__create_directory', description: 'Create a directory and all parent directories recursively. No error if the directory already exists.', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Absolute path of directory to create' } }, required: ['path'] } },
            { name: 'filesystem__list_directory', description: 'List entries in a directory. Returns [FILE] or [DIR] prefix for each entry. Does not recurse into subdirectories.', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Absolute path of directory to list' } }, required: ['path'] } },
            { name: 'filesystem__directory_tree', description: 'Return a recursive tree structure of a directory up to a configurable depth. Useful for understanding project layout.', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Root path for the tree' }, depth: { type: 'number', description: 'Maximum depth to recurse (default: 3)' } }, required: ['path'] } },
            { name: 'filesystem__move_file', description: 'Move or rename a file or directory. Fails if the destination already exists.', input_schema: { type: 'object', properties: { source: { type: 'string', description: 'Current path of file or directory' }, destination: { type: 'string', description: 'New path for the file or directory' } }, required: ['source', 'destination'] } },
            { name: 'filesystem__search_files', description: 'Search for files matching a glob pattern recursively from the given path. Returns matching file paths.', input_schema: { type: 'object', properties: { path: { type: 'string', description: 'Base directory to search from' }, pattern: { type: 'string', description: 'Glob pattern to match (e.g., "**/*.ts", "*.json")' }, excludePatterns: { type: 'array', items: { type: 'string' }, description: 'Patterns to exclude from results' } }, required: ['path', 'pattern'] } },
        ...
        },
        {
          ...
          tools: [
            { name: 'postgres__query', description: 'Execute a read-only SQL query (SELECT, EXPLAIN, SHOW). Returns results as JSON rows. Supports parameterized queries with $1, $2, etc.', input_schema: { type: 'object', properties: { sql: { type: 'string', description: 'SQL query to execute (SELECT, EXPLAIN, or SHOW only)' }, params: { type: 'array', items: { type: 'string' }, description: 'Parameterized values for $1, $2, etc. placeholders' } }, required: ['sql'] } },
            { name: 'postgres__execute', description: 'Run a write SQL statement (INSERT, UPDATE, DELETE, CREATE, ALTER, DROP). Returns the number of affected rows. Always use parameterized queries for user-provided values.', input_schema: { type: 'object', properties: { sql: { type: 'string', description: 'SQL statement to execute' }, params: { type: 'array', items: { type: 'string' }, description: 'Parameterized values for $1, $2, etc.' } }, required: ['sql'] } },
            { name: 'postgres__list_schemas', description: 'List all schemas in the connected database with their descriptions and table counts.', input_schema: { type: 'object', properties: {} } },
            { name: 'postgres__list_tables', description: 'List tables in a schema with row counts, size estimates, and descriptions. Defaults to the "public" schema.', input_schema: { type: 'object', properties: { schema: { type: 'string', description: 'Schema name (default: "public")' } } } },
            { name: 'postgres__describe_table', description: 'Get detailed column definitions for a table including name, type, nullable, default value, primary keys, foreign keys, unique constraints, and check constraints.', input_schema: { type: 'object', properties: { schema: { type: 'string', description: 'Schema name (default: "public")' }, table: { type: 'string', description: 'Table name to describe' } }, required: ['table'] } },
            { name: 'postgres__explain_query', description: 'Run EXPLAIN ANALYZE on a query and return the execution plan with timing and row estimates. The query is executed within a rolled-back transaction so no data is modified.', input_schema: { type: 'object', properties: { sql: { type: 'string', description: 'SQL query to analyze' }, params: { type: 'array', items: { type: 'string' }, description: 'Parameterized values' }, format: { type: 'string', enum: ['text', 'json', 'yaml'], description: 'Output format (default: text)' } }, required: ['sql'] } },
          ],
          resources: [
        ...
        },
        {
          ...
          compression_state: isMidThreshold ? 'compressed' : 'visible',
          tools: [
            { name: 'slack__send_message', description: 'Post a message to a Slack channel or direct message. Use the channel ID (not name). Supports Slack mrkdwn formatting including bold, italic, code blocks, and links. For threaded replies, include the thread_ts parameter.', input_schema: { type: 'object', properties: { channel: { type: 'string', description: 'Channel ID (e.g., "C024BE91L") — use list_channels to find IDs' }, text: { type: 'string', description: 'Message text with optional Slack mrkdwn formatting' }, thread_ts: { type: 'string', description: 'Thread timestamp to reply in a thread' }, unfurl_links: { type: 'boolean', description: 'Whether to unfurl URL previews (default: true)' } }, required: ['channel', 'text'] } },
            { name: 'slack__list_channels', description: 'List workspace channels with IDs, names, topics, purposes, and member counts. Results are paginated with cursor-based navigation.', input_schema: { type: 'object', properties: { types: { type: 'string', description: 'Comma-separated channel types: public_channel, private_channel, im, mpim' }, limit: { type: 'number', description: 'Max results per page (default: 100, max: 1000)' }, cursor: { type: 'string', description: 'Pagination cursor from previous response' }, exclude_archived: { type: 'boolean', description: 'Exclude archived channels (default: false)' } } } },
            { name: 'slack__search_messages', description: 'Full-text search across all messages the authenticated user has access to. Supports Slack search modifiers: in:#channel, from:@user, before:YYYY-MM-DD, after:YYYY-MM-DD, has:link, has:reaction, is:thread.', input_schema: { type: 'object', properties: { query: { type: 'string', description: 'Search query with optional modifiers' }, count: { type: 'number', description: 'Number of results per page (default: 20, max: 100)' }, sort: { type: 'string', enum: ['score', 'timestamp'], description: 'Sort by relevance or recency' }, sort_dir: { type: 'string', enum: ['asc', 'desc'], description: 'Sort direction' } }, required: ['query'] } },
