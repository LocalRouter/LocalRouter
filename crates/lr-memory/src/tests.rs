...
fn session_manager_creates_new_session() {
    ...
    let (path, is_new) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
    assert!(is_new);
...
fn session_manager_reuses_active_session() {
    ...
    let (_, is_new1) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
    assert!(is_new1);

    let (_, is_new2) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
    assert!(!is_new2);
}
...
fn session_manager_isolates_clients() {
    ...
    let (path1, _) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
    let (path2, _) = mgr.get_or_create_session("client-2", &dir, "test content", "test-folder");
...
fn session_manager_records_conversation() {
    ...
    mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
...
fn touch_by_path_updates_activity() {
    ...
    let (path, _) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
...
fn detect_conversation_new_session() {
    ...
    let ctx = mgr.detect_conversation_for_both_mode(
        "client-1",
        &messages,
        &dir,
        "test content",
...
fn detect_conversation_continuation() {
    ...
    let ctx1 = mgr
        .detect_conversation_for_both_mode(
            "client-1",
            &messages1,
            &dir,
            "test content",
...
async fn transcript_create_and_append() {
    ...
    // Create session file
    let path = active_dir.join("test-session.md");
...
async fn transcript_format_snapshot() {
    ...
    let writer = TranscriptWriter::new();
    let path = active_dir.join("87286ef5-abcd-1234.md");
    writer.create_session_file(&path).await.unwrap();
...
async fn transcript_restore_from_archive() {
    ...
    // Restore
    let restored = writer
        .restore_from_archive("test-session", &active_dir, &archive_dir)
        .await
        .unwrap();
...
fn memory_service_ensure_client_dir() {
    ...
    assert!(client_dir.join("active").exists());
...
fn session_manager_removes_expired_before_creating_new() {
    ...
    let (_path1, _) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");

    // Wait for it to expire
    std::thread::sleep(Duration::from_millis(10));

    // Should create a new session (not return expired one)
    let (_, is_new) = mgr.get_or_create_session("client-1", &dir, "test content", "test-folder");
    assert!(is_new);
...
fn compaction_stats_counts_session_files() {
    ...
    std::fs::write(client_dir.join("active/aaa.md"), "content a").unwrap();
    std::fs::write(client_dir.join("active/bbb.md"), "content b").unwrap();
...
fn compaction_stats_excludes_active_session() {
    ...
    let (active_path, _) = svc.session_manager.get_or_create_session(
        "test-client",
        &client_dir.join("active"),
        "test content",
        "test-folder",
    );
    ...
    std::fs::write(client_dir.join("active/expired.md"), "expired").unwrap();
...
async fn force_compact_moves_expired_sessions() {
    ...
    std::fs::write(active_dir.join("expired1.md"), "expired 1").unwrap();
    std::fs::write(active_dir.join("expired2.md"), "expired 2").unwrap();
...
async fn compact_session_with_llm_creates_summary() {
    ...
    let session_path = active_dir.join("test-session.md");
...
async fn compact_session_without_llm_archives_only() {
    ...
    let outcome =
        crate::compaction::compact_session(&session_path, &archive_dir, None, None, false)
            .await
            .unwrap();

    assert!(matches!(
        outcome,
        crate::compaction::CompactionOutcome::ArchivedOnly
    ));
    assert!(archive_dir.join("test-session.md").exists());
    assert!(!archive_dir.join("test-session-summary.md").exists());
}
